...
public class MainMenuUI {
    ...
    public StackPane createMenu(double width, double height) {
        ...
        // 2. Üst Kısım: Profil Bölümü
        VBox topSection = new VBox(15);
        topSection.setAlignment(Pos.CENTER);
        topSection.setPadding(new Insets(60, 0, 0, 0));

        ImageView avatarView = new ImageView(Assets.avatar1);
        avatarView.setFitWidth(100);
        avatarView.setFitHeight(110);
        avatarView.setStyle("-fx-effect: dropshadow(gaussian, #00d2ff, 15, 0.5, 0, 0);");

        Label nameLabel = new Label(Main.nicknameAtTextBox);
        nameLabel.setFont(Font.font("Orbitron", FontWeight.BLACK, 24));
        nameLabel.setTextFill(Color.WHITE);
        nameLabel.setStyle("-fx-effect: dropshadow(gaussian, black, 2, 1, 1, 1);");

        VBox profileBox = new VBox(5, avatarView, nameLabel);
        profileBox.setAlignment(Pos.CENTER);
        profileBox.setPadding(new Insets(10));
        profileBox.setStyle("-fx-background-color: rgba(255,255,255,0.05); -fx-background-radius: 15;");

        topSection.getChildren().add(profileBox);

        // 3. Alt Kısım: Butonlar Bölümü
        VBox bottomSection = new VBox(20);
        bottomSection.setAlignment(Pos.CENTER);
        bottomSection.setPadding(new Insets(0, 0, 40, 0));
        ...
        createLobbyBtn.setOnAction(e -> controller.CreateLobby());
        joinLobbyBtn.setOnAction(e -> controller.JoinLobby());
